...
km_driven = st.slider('Number of kms Driven', 10 , 200000)

fuel = st.selectbox('Select Type of Fuel', cars_data['fuel'].unique())

seller_type = st.selectbox('Select Type of Seller', cars_data['seller_type'].unique())
...
mileage = st.slider('Drop Mileage', 10 , 45)
...
if st.button("Get Car Price"):
  ...
  st.markdown(f'**Estimated Car Price:** ₹{int(car_price[0])}')
